...
class TransactionService:
    ...
    async def create_transaction(self, user: Dict[str, Any], tx_data: Dict[str, Any]) -> Tuple[int, Dict[str, Any]]:
        ...
        if not recipient_name:
            return 400, {"error": "Recipient name is required."}

        recipient_phone = str(tx_data.get("recipientPhone", "")).strip() or None
        is_new_recipient = bool(tx_data.get("isNewRecipient", False))

        # 1. Evaluate risk server-side first to enforce security decisions
        eval_status, eval_res = await self.evaluate_transaction({
            "recipientName": recipient_name,
            "recipientPhone": recipient_phone,
            "amount": amount,
            "paymentType": tx_data.get("type"),
            "note": tx_data.get("note"),
            "isNewRecipient": is_new_recipient,
        })

        decision = eval_res.get("decision", "ALLOW") if eval_status == 200 else "ALLOW"
        safety_score = eval_res.get("safetyScore", 90) if eval_status == 200 else 90
        risk_level = eval_res.get("riskLevel", "LOW") if eval_status == 200 else "LOW"
        reasons = eval_res.get("humanReasons", []) if eval_status == 200 else []
        technical_details = eval_res.get("technicalDetails") if eval_status == 200 else None

        # 2. Check sender balance ONLY if transaction is allowed/challenged
        current_sender_user = self.user_repo.find_by_id(user_id) or user
        sender_balance = float(current_sender_user.get("balance", 45000.0))

        if decision != "BLOCK" and sender_balance < amount:
            return 400, {"error": f"Insufficient account balance (Available: ₹{sender_balance:,.2f}, Requested: ₹{amount:,.2f})."}

        now_iso = datetime.utcnow().isoformat()
        tx_id = f"tx-{int(time.time() * 1000)}"

        status_str = "BLOCKED" if decision == "BLOCK" else ("CHALLENGED" if decision == "CHALLENGE" else "COMPLETED")
        ...
        new_tx = {
            ...
            "type": tx_data.get("type") or "PHONE",
            "status": status_str,
            "decision": decision,
        ...
        # 3. Save transaction in SQLite
        self.tx_repo.create(new_tx)

        # 4. Atomic Balance & Recipient Handling
        if decision != "BLOCK":
            # Deduct sender balance
            new_sender_bal = self.user_repo.update_balance(user_id, -amount)
            new_tx["senderNewBalance"] = new_sender_bal

            # Credit recipient if registered user
            recipient_user = None
            if recipient_phone:
                recipient_user = self.user_repo.find_by_phone(recipient_phone)
            if not recipient_user and recipient_name:
                recipient_user = self.user_repo.find_by_identifier(recipient_name)

            if recipient_user and recipient_user["id"] != user_id:
                self.user_repo.update_balance(recipient_user["id"], +amount)
                # Create corresponding credit transaction entry for recipient
                rec_tx_id = f"tx-rec-{int(time.time() * 1000)}"
                self.tx_repo.create({
                    "id": rec_tx_id,
                    "userId": recipient_user["id"],
                    "recipientName": user.get("fullName") or "Sender",
                    "recipientPhone": user.get("phone"),
                    "amount": amount,
                    "note": f"Received from {user.get('fullName', 'Sender')}",
                    "category": "Income",
                    "type": tx_data.get("type") or "PHONE",
                    "status": "COMPLETED",
                    "decision": "ALLOW",
                    "safetyScore": 99,
                    "riskLevel": "LOW",
                    "reasons": ["Incoming transfer received"],
                    "technicalDetails": None,
                    "timestamp": now_iso,
                    "isNewRecipient": False,
                })
        else:
            # Generate Security Alert for Blocked Transaction
            alert_repository.create({
                "id": f"alt-{int(time.time() * 1000)}",
                "userId": user_id,
                "title": "High-Risk Transfer Blocked",
                "message": f"SentinelFin blocked a transfer of ₹{int(amount):,} to {recipient_name} due to elevated fraud indicators.",
                "severity": "critical",
                "isRead": False,
                "actionTaken": "Blocked by Zero-Trust Risk Engine",
                "timestamp": now_iso
            })

        # 5. Sync to Neo4j Graph Database
        try:
